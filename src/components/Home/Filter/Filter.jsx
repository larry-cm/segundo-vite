import { useState, useContext } from 'react'
import { Filter } from 'lucide-react'

import GifContext from '@/context/GifContext'

export default function FilterBtn () {
  const [open, setOpen] = useState(false)
  const [modeSelected, setModeSelected] = useState(() => window.localStorage.getItem('lastMode') || '')
  const { setMode } = useContext(GifContext)
  function handleClick () {
    setOpen(!open)
  }

  function handleChange (e) {
    const val = e.target.value
    setMode(val)
    setModeSelected(val)
    window.localStorage.setItem('lastMode', val)
  }

  return (
    <>
      <button
        onClick={handleClick}
        type='button'
        className={`px-4 py-2 border border-primary rounded-lg duration-300 hover:bg-primary  font-medium  transition text-text capitalize cursor-pointer flex items-center gap-4  ${open ? 'bg-primary' : ''}`}
      >
        <Filter className=' text-text ' />
        <span>Filtrar</span>
      </button>
      {
        open && (
          <article className='absolute z-10 top-16 right-0 h-fit p-6 bg-fondo border border-primary rounded-lg shadow-2xl '>
            <header>
              <h4 className='text-text font-medium '>Disponible para filtrar</h4>
            </header>

            <div className='flex flex-col gap-4 pt-4'>

              <label className='cursor-pointer'>
                <input
                  type='radio'
                  name='mode'
                  onChange={handleChange}
                  value='gifs'
                  className='peer hidden'
                  checked={modeSelected === 'gifs'}
                />
                <div className='text-text font-medium transition duration-200 text-sm text-center hover:scale-105 hover:bg-secondary rounded p-2 peer-checked:bg-secondary peer-checked:scale-105'>
                  Giphy Public API
                </div>
              </label>

              <label className='cursor-pointer'>
                <input
                  type='radio'
                  name='mode'
                  onChange={handleChange}
                  value='stickers'
                  checked={modeSelected === 'stickers'}
                  className='peer hidden'
                />
                <div className='text-text font-medium transition duration-200 text-sm text-center hover:scale-105 hover:bg-secondary rounded p-2 peer-checked:bg-secondary peer-checked:scale-105'>
                  Giphy Sticker API
                </div>
              </label>

            </div>

          </article>
        )
      }

    </>
  )
}
